...
def estado_maquina(request, id):
    ...
    paradas = [{
        ...
        'duracion': p.duracion(),
        'turnos': [t for t in p.turnos()],
        'duracion_por_turno': [dpt for dpt in p.duracion_por_turno()],
        'rendimiento': p.rendimiento(),
        'rendimiento_por_turno': [rpt for rpt in p.rendimiento_por_turno()],
...
def crear_parada_ProdBD(request):
    ...
    tiempo_anterior = int(parada_tiempo - tiempo)
    ...
    cursor = conn.cursor()
    try:
        xDescripcion = get_descripcion_prodDB(xIdParada, xIdTipo, cursor)
        xIdPos = get_posicion_prodDB(xIdOF, xIdTipo, cursor)
